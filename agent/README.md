...
自然语言链路保持一次模型调用，并把判断与执行明确分开：

```text
能力目录 → 动态系统 Prompt → 单次 GLM 结构化识别 → 本地契约校验
        → 来源与安全策略 → 独立能力处理器 → 可替换执行端口
```

- `capabilities.py` 是支持意图、动作、参数、枚举值和示例的单一事实来源。
- `prompting.py` 从能力目录生成 Prompt，明确处理否定、假设、转述、歧义、医疗边界和应急优先级。
- `intent_contract.py` 只接受允许的字段、枚举、目标和参数；低置信度、非法 JSON、否定或不应执行的表达统一降为未知意图。
- `skills/` 中每项能力有独立处理器；通话、留言、提醒、记录、床控、天气和媒体通过 `ports.py` 中的窄接口执行。
- 当前端口使用内存模拟适配器，不使用 SQL；替换某一真实设备或业务系统时不需要改动模型输出契约。
...
├── docs/                  # Agent 架构说明
├── src/care_bed_agent/
│   ├── capabilities.py    # 能力目录
│   ├── prompting.py       # 动态系统 Prompt
│   ├── intent_contract.py # 模型输出白名单与归一化
│   ├── ports.py           # 可替换执行端口
│   ├── skills/            # 细粒度能力处理器
│   └── ...                # 分流、策略、状态与 API
└── tests/                 # 离线行为测试与手动真实模型冒烟
```
...
对话历史只用于指代与连续表达，只有最后一条用户消息能够触发动作。否定、假设、转述、医疗诊断与用药调整不会执行；应急求助优先于同句中的其他需求。
