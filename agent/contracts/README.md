...
| `POST` | `/api/v1/bedside/messages` | 提交床侧自然语言及可选页面上下文 | Agent 路径 |
...
床侧消息可携带可选 `history`：最多 16 条按时间顺序排列的消息，`role` 仅支持 `user` 和 `assistant`，每条 `content` 为 1–500 个字符。历史消息只帮助模型理解当前最后一句话，不能作为新的执行命令。演示页刷新后应使用新的 `actor_id` 并从空历史开始；接口不会保存或返回长期会话。
...
当前业务数据、天气、通话和媒体均为演示实现，服务重启后内存数据会恢复为演示种子状态。页面级短期对话由前端内存维护，刷新页面即清空。完整字段定义见 `openapi.json`。
